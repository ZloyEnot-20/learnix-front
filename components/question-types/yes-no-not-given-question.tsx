...
export function YesNoNotGivenQuestion({ question, answer, onAnswerChange }: YesNoNotGivenQuestionProps) {
  return (
    <div className="mb-6 p-4 bg-white rounded border">
      ...
      <RadioGroup value={answer} onValueChange={(value) => onAnswerChange(question.id, value)} className="ml-11">
        <div className="flex items-center space-x-2 mb-2">
          ...
          <Label htmlFor={`q${question.id}-yes`} className="cursor-pointer text-sm">
            YES
          </Label>
        </div>
        <div className="flex items-center space-x-2 mb-2">
          <RadioGroupItem value="NO" id={`q${question.id}-no`} />
          <Label htmlFor={`q${question.id}-no`} className="cursor-pointer text-sm">
            NO
          </Label>
        </div>
